...
target_store = sys.argv[3] if len(sys.argv) > 3 else None # Name of the existing Mosaic in GeoServer e.g.: METOFFICE-GLO-SST-L4-NRT-OBS-SKIN-DIU_temperature
...
lat_nc = fid.variables['latitude']
lats = np.asarray(lat_nc[:])
lon_nc = fid.variables['longitude']
lons = np.asarray(lon_nc[:])
time_nc = fid.variables['time_counter']
...
for subdataset in dataset.GetSubDatasets():
    ...
    if re.match('\[.*\] .*'+vname+'.* \(.*\)', subdataset[1]):
        ...
        for i in range(1, rasters):
            ...
            time_units = lc_data.GetMetadata()["time_counter#units"] # Trying to extract the time origin from the metadata
            ...
            st=datetime.datetime(year=int(start_time[0]), month=int(start_time[1]), day=int(start_time[2]), hour=0) + datetime.timedelta(hours=int(time))
